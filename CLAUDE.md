...
| `.claude/rules/case.md` | `aquaflux/case/**`, `validation/*/case.yaml` | a whole case described in one YAML file: the `CaseSpec` core plus the physics and drive discriminators, one physical kind per boundary patch (closures and wall set derived, never restated), the fluid stated once with exactly one viscosity, why the YAML parse is 1.2 and refuses duplicate keys, and what is built (read + check against the mesh's topology) versus not yet (building the assemblers, the solver section) |
