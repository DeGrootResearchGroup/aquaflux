...
| Compiled host loops | **Numba** | a loop whose value is in the work it SKIPS (a ray walked to its first hit) cannot be traced — a traced loop pays its worst case on every item — and written as numpy passes it pays per step for every item still in flight. Its consumer is the triangle grid's walk (`aquaflux/radiation/grid_walk.py`), 17-37x the numpy walk it replaced, with identical answers (`.claude/rules/radiation.md`). A core dependency since 2026-09-26, by the project owner's decision over keeping it optional. |
