...
def test_line_search_fails_in_the_convective_regime_even_with_best_preconditioner() -> None:
    ...
    A stronger preconditioner (mass-scaled Schur + convection-velocity AMG) extends the line-search reach
...
@pytest.mark.slow
def test_mass_scaled_schur_reaches_beyond_the_a_p_schur() -> None:
    """The mass-scaled pressure Schur carries the solve past the Reynolds number where SIMPLE stalls.
    ...
    continuation. The mass-scaled Schur's frozen, velocity-independent scaling (``schur_scaling="msimple"``)
    ...
    swap; the test asserts the positive: the mass-scaled-Schur continuation converges at this Reynolds
    number.
    """
...
@pytest.mark.slow
def test_mass_scaled_schur_matches_a_p_schur_at_moderate_reynolds() -> None:
    """The mass-scaled Schur is a drop-in for the ``a_P`` one where that already works: both converge, and the mass-scaled
    ...
    continuation = momentum_continuation(_channel(32, 24, 2e-3), schur_scaling="msimple")
...
def test_mass_scale_tracks_the_characteristic_speed() -> None:
    """The mass-scaled Schur's ``k`` auto-calibrates to the operating scale, with no unit-speed assumption.
    ...
    calibrates itself — the case that previously needed a manual ``mass_scale``.
    """
    ...
    def scale_at(u_in):
        ...
        preconditioner = BlockPreconditioner.build(assembler, schur_scaling="msimple")
        ...
        return float(preconditioner._mass_scale(state))
...
@pytest.mark.slow
def test_mass_scale_auto_calibration_converges_at_non_unit_speed() -> None:
    """The auto-calibrated ``k`` carries the mass-scaled-Schur solve at a characteristic speed far from one,
    with no manual ``mass_scale`` — the unit-speed assumption is gone. A deliberately mis-scaled
