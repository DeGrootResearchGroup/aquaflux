...
class _LinearSource(VolumeSource):
    """``+ rate * phi`` per unit volume — a production-like source that feeds the field back.

    Stands in for the k-production of a coupled RANS residual, whose eddy viscosity is a live function
    of ``k``: a source proportional to the solved field, whose exact linearization *lowers* the
    equation's diagonal and can take it negative.
    """
...
def _diffusion_with_source(mesh, geometry, source_rate=None):
    """A pure-diffusion scalar, optionally with a ``+ rate * phi`` volume source.

    Pure diffusion so the shift diagonal is exactly ``diag(J)`` in the absence of the source (see the
    test below), which makes every comparison here an equality rather than a tolerance.
    """
    assembler = ResidualAssembler.build(
        mesh,
        geometry,
        ...
        source_operators=() if source_rate is None else (_LinearSource(source_rate),),
    )
    return assembler.residual


def _destabilized(mesh, geometry, reference):
    """A pure-diffusion residual and a source rate strong enough to invert its interior diagonal.

    The rate is taken from the diffusion diagonal itself (twice it), so the premise "the source takes
    the diagonal negative" holds by construction rather than by a hand-tuned constant that a mesh or
    conductance change could quietly falsify.
    """
    plain = _diffusion_with_source(mesh, geometry)
    diagonal = jnp.diag(jax.jacfwd(plain)(reference))
    rate = 2.0 * diagonal / geometry.cell.volume
    return plain, rate, _diffusion_with_source(mesh, geometry, rate)


def _interior_cells(mesh):
    """Cells owning no boundary face — the ones whose reaction diagonal is the source alone."""
    boundary = mesh.face_cells.owner[~mesh.face_cells.interior]
    interior = np.ones(mesh.n_cells, dtype=bool)
    interior[np.asarray(boundary)] = False
    return interior


def test_a_destabilizing_source_grows_the_shift_and_is_dropped_from_the_preconditioner() -> None:
    """One reaction diagonal, two consumers, opposite sign treatments (#312).

    A source proportional to the solved field lowers the equation's diagonal and can take it negative.
    The preconditioner must **drop** that (``maximum(., 0)``): an indefinite operator is not one an
    aggregation V-cycle can coarsen, and a softer preconditioner costs only iterations. The
    pseudo-time shift must instead keep its **magnitude**: a row destabilized by its own source is
    exactly a row whose local pseudo-timestep has to be smaller, i.e. whose shift has to be larger.

    Clamping in the shared assembly served both from one array, so the shift silently got the
    preconditioner's answer — and ``J + beta d`` then near-cancels in precisely the cells the damping
    exists for.
    """
    mesh = structured_grid_2d(6, 5, lx=4.0, ly=1.0, named_boundaries=True)
    geometry = mesh.geometry()
    reference = jnp.zeros(mesh.n_cells)
    plain, rate, destabilized = _destabilized(mesh, geometry, reference)
    interior = _interior_cells(mesh)
    gamma, no_flux = jnp.full(mesh.n_cells, GAMMA), jnp.zeros(mesh.n_faces)

    # The premise: the source really does take the interior diagonal negative.
    diag_j = np.asarray(jnp.diag(jax.jacfwd(destabilized)(reference)))
    assert np.all(diag_j[interior] < 0.0)

    shift = {
        name: np.asarray(
            scalar_transport_shift_diagonal(mesh, geometry, gamma, no_flux, residual, reference)
        )
        for name, residual in (("plain", plain), ("destabilized", destabilized))
    }
    # An interior cell has no boundary stiffness, so its whole reaction diagonal is the source: the
    # shift picks up its magnitude rather than losing it to a clamp.
    added = np.asarray(rate * geometry.cell.volume)[interior]
    assert np.allclose(shift["destabilized"][interior], shift["plain"][interior] + added)

    # The preconditioner keeps the M-matrix: same fine-level diagonal as without the source at all.
    fine = {
        name: np.asarray(
            scalar_transport_preconditioner(mesh, geometry, gamma, no_flux, residual, reference)
            .hierarchy.levels[0]
            .diagonal
        )
        for name, residual in (("plain", plain), ("destabilized", destabilized))
    }
    assert np.allclose(fine["destabilized"][interior], fine["plain"][interior])


def test_source_feedback_gives_the_shift_of_the_equation_that_carries_the_term() -> None:
    """``source_feedback`` folds in a diagonal term the supplied residual cannot show (#312).

    The k shift of a **coupled** RANS solve is built from the frozen-closure k residual, in which
    ``nu_t`` is a constant array and the production ``nu_t S**2`` therefore contains no ``k`` at all.
    The equation actually being marched recomputes ``nu_t = a_1 k / max(a_1 omega, S F_2)``, so that
    production is proportional to ``k`` and sits on the row's diagonal. ``source_feedback`` supplies
    that term in closed form, and this pins the equivalence exactly: the shift built from the residual
    *without* the term plus its feedback rate is the shift of the residual that **has** it.
    """
    mesh = structured_grid_2d(6, 5, lx=4.0, ly=1.0, named_boundaries=True)
    geometry = mesh.geometry()
    reference = jnp.zeros(mesh.n_cells)
    plain, rate, live = _destabilized(mesh, geometry, reference)
    gamma, no_flux = jnp.full(mesh.n_cells, GAMMA), jnp.zeros(mesh.n_faces)

    folded = scalar_transport_shift_diagonal(
        mesh,
        geometry,
        gamma,
        no_flux,
        plain,
        reference,
        source_feedback=rate * geometry.cell.volume,
    )
    carried = scalar_transport_shift_diagonal(mesh, geometry, gamma, no_flux, live, reference)
    assert jnp.array_equal(folded, carried)
    # ...and it is not vacuous: the term genuinely moves the shift.
    plain_shift = scalar_transport_shift_diagonal(mesh, geometry, gamma, no_flux, plain, reference)
    assert not jnp.allclose(folded, plain_shift)
