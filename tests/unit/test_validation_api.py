...
``**strategy_kwargs`` and forwards them to whichever continuation builder it is given, so *every*
...
to reach: given an explicit ``strategy`` or a ``RefreshPolicy(builder=...)``, a continuation setting
is refused rather than dropped in silence, which is how a ``refresh_preconditioner=`` meant for a
