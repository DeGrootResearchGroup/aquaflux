"""Unit tests for the flow system's named blocks over the shared flat-vector layout."""
...
def test_pack_unpack_round_trips() -> None:
    layout = flow_state_layout(dim=2, n_cells=3)
...
def test_size_and_zeros() -> None:
    layout = flow_state_layout(dim=3, n_cells=4)
...
def test_works_in_3d() -> None:
    layout = flow_state_layout(dim=3, n_cells=2)
...
def test_the_blocks_are_named_so_a_coupled_state_can_nest_them() -> None:
    layout = flow_state_layout(dim=3, n_cells=4)
    assert layout.names == ("velocity", "pressure")
    assert layout.slice_of("velocity") == slice(0, 12)
    assert layout.slice_of("pressure") == slice(12, 16)
