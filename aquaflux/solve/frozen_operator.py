...
def equilibrate_cell_major(
    ...
    """Symmetrically equilibrate an assembled coupled block matrix and reorder it cell-major.

    The two conditioning transforms the indefinite Rhie--Chow saddle needs before a multigrid smoother
    acts on it:
    ...
    * **Symmetric square-root-diagonal equilibration** ``D A D`` with ``D = diag(1/sqrt(|diag A|))`` — the
      ...
      them so the smoother's pivots stay well conditioned.
    * **Cell-major reordering** — the per-cell fields ``[u, v, (w,) p, k, omega]`` interleaved (rather than
    ...
    ⚠️ **On why the interleaving helps, be careful what is claimed.** The published saddle-point incomplete
    factorizations that report *stable* factorizations number velocity first and pressure last — the
    opposite grouping — because eliminating the velocities first fills the pressure diagonal before it is
    reached (Konshin, Olshanskii & Vassilevski, *SIAM J. Sci. Comput.* 37(5), 2015). What *is* proven is
    narrower and is about pairing rather than grouping: for F-matrices, an ordering in which each pressure
    node is eliminated together with a connected velocity node is numerically stable (de Niet & Wubs,
    *IMA J. Numer. Anal.* 29(1), 2009), and cell-major is a coarse approximation of that. Measured on a
    coupled velocity--pressure saddle at zero fill, the interleave wins and pressure-last loses badly,
    which is consistent with both: pressure-last relies on the Schur fill that a zero-fill factorization
    discards. Read the ordering as sound *for zero fill* and as an open question for a factorization that
    keeps fill.
    ...
    cell_major : scipy.sparse.csr_matrix
        The equilibrated matrix ``(D A D)`` reordered cell-major, with canonical (sorted) indices.
    scale : np.ndarray
