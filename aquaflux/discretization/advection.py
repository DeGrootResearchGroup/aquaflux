...
class AdvectionFlux(FaceFluxOperator):
    ...
    mass_flux : jnp.ndarray
        ...
        ``(u . n) A`` (a verification-only construction, which is why it is not a library function);
        in a coupled momentum solve it is the Rhie--Chow face flux, the *same* flux that closes
        continuity — sharing it is what keeps the momentum convection and continuity discretely
        consistent. The operator only reads ``mdot_f``; how it was formed is the caller's concern.
