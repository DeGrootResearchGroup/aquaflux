...
### This mesh is skew-free, and the gradient sweep is five orders past what it needs

The corrected-gradient system is solved by a fixed number of preconditioned-Richardson sweeps, and the
count needed is set by the mesh's non-orthogonality. This mesh is a graded but orthogonal hexahedral
block, so the skewness correction is essentially absent: measured over its 23040 cells, the sweep
contracts at `rho = 5.06e-15`, which means the **first** sweep already reaches machine precision and
every later one refines an answer that is exact. The count reaching a relative gradient error of
`1e-10` is 1, as is the count reaching `1e-4`.

That was measured by reading `validation/bfs3d_species/of_case/constant/polyMesh`, which is this
case's own geometry — the species case reuses it — and is the copy checked into the repository, since
`of_case/constant/polyMesh` here is generated by running OpenFOAM and is absent from a fresh checkout.
The two are the same mesh: 23040 cells and 66368 interior faces, matching the face counts quoted for
this case elsewhere.

The case nonetheless runs the library default of **4**, spending three operator applies per
reconstruction — on every residual evaluation and every Jacobian–vector product — to no effect. That
default is deliberately not changed here, because the sweep count also sets how far the residual's
Jacobian reaches across the cell graph and the case's probing reach is matched to it, so moving it is a
change to the discretization that the reattachment result would have to be re-validated against. It is
recorded because it is the clearest available example of why the count belongs to the mesh: the same
`4` that is five orders of over-resolution here is *short* of `1e-4` on a mesh at 40 % grid
perturbation. `CorrectedGreenGauss.calibrated(mesh, geometry)` is how a mesh is asked rather than
assumed.
