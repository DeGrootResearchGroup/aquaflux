...
def _run_beta(beta: int) -> dict:
    ...
    case = betadir / "case"
    # Reuse an existing OpenFOAM run if present (its iteration counts are fixed by the
    # external code); only the aquaflux side changes when the flow module is edited.
    have_of = (case / "log.checkMesh").exists() and (betadir / "nonorth_2").exists()
    if have_of:
        print(f"\n=== beta = {beta} deg  (reusing existing OpenFOAM run) ===")
    else:
        if betadir.exists():
    ...
    # nonorth_1 (1 non-orthogonal corrector) converges at every skew angle, so it is the
    # reference OF field; nonorth_2 diverges at the highest non-orthogonality. At convergence
    # the two agree (the deferred correction has fully caught up), so this is consistent.
    ref = betadir / "nonorth_1"
    mesh = read_openfoam(ref)
    geom = mesh.geometry()
    imp_assembler = C._build(mesh, geom, CorrectedGreenGauss())
    imp_hist, imp_state, _ = C._solve(imp_assembler, 12, label=f"impl b{beta}")
    lag_hist, _, _ = C._solve(
        C._build(mesh, geom, C.LaggedGradient(inner=CorrectedGreenGauss())), 60, label=f"lag b{beta}"
    )

    # velocity accuracy vs the OpenFOAM nOrtho=2 converged field (cell-mapped by centroid)
    from scipy.spatial import cKDTree

    of_time = C._latest_time(ref)
    U_of = C._read_internal_field(of_time / "U")[:, :2]
    C_of = C._read_internal_field(case / "0" / "C")[:, :2]
    perm = cKDTree(C_of).query(np.asarray(geom.cell.centroid))[1]
    U_aq, _ = (np.asarray(x) for x in imp_assembler.unpack(imp_state))
    err = np.linalg.norm(U_aq - U_of[perm], axis=1)
    u_rms, u_linf = float(np.sqrt(np.mean(err**2)) / C.U_LID), float(err.max() / C.U_LID)
    print(f"  velocity vs OpenFOAM: RMS {100 * u_rms:.2f}%, max {100 * u_linf:.1f}%")
    return {
        "beta": beta,
        "nonortho": nonortho,
        "of": of,
...
def _figure(results):
    ...
    # --- panel 1: convergence (iterations to converge) ---
    ...
    ax.set_title("Convergence vs non-orthogonality")
    ax.grid(True, which="both", alpha=0.25)
    ax.legend(fontsize=8, loc="center left")

    # --- panel 2: accuracy (aquaflux vs OpenFOAM velocity, % of lid speed) ---
    ax2.plot(xs, [100 * d["u_rms"] for d in r], "o-", color="#1b7837", lw=2.4, ms=9,
             label="RMS  |U$_{aqua}$ - U$_{OF}$| / U$_{lid}$")
    ax2.plot(xs, [100 * d["u_linf"] for d in r], "s--", color="#7aa36a", lw=1.6,
             label="max (single cell, the singular lid corner)")
    ax2.set_ylim(bottom=0)
    ax2.set_xlabel("mesh non-orthogonality (deg)")
    ax2.set_ylabel("velocity difference vs OpenFOAM (% of lid speed)")
    ax2.set_title("Accuracy vs non-orthogonality")
...
def _report(results):
    ...
    rows = "\n".join(
        ...
        f"{100 * d['u_rms']:.2f} | {100 * d['u_linf']:.1f} |"
        for d in r
    )
    md = f"""# Skew-angle sweep: iterations and accuracy vs mesh non-orthogonality
