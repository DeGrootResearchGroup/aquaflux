...
> first: peel the dated/historical content into a new `.claude/notes/solve-march-log.md` (outside the
...
- **`march.py` — BUILT (`forward_march`, `StepReport`/`MarchResult`, `RefreshTrigger`/`CycleGrowthTrigger`):
  ...
  - **`solve_coupled` does NOT declare `grow`, and must not be "fixed" to (binding).** It is a
    ...
    lives; pinned call-for-call by `test_globalization_knobs_still_reach_the_continuation_builder`.
